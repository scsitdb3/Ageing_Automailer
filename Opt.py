...
"Left JOIN Dealer_Status_for_Ageing_Automation B on a.Dealerid = B.Dealerid ",conn)
...
sug_dis = pd.read_sql_query('select *from automation..uad_Suggesting_Discount_Master_Gainer where brandid=?',conn,params=int(selected_brand_id),) 
...
with tab1:
    ...
    with tab2:
        ...
        selected_ids_df = (
            loc_df[(loc_df["Dealer"].isin(selected_deale)) & (loc_df["Brand"] == brand)][["Dealer","Brandid", "Dealerid"]]
            .dropna()
           # .astype(int)
        )

        if st.button("Generate NonMoving Report") and not selected_ids_df.empty:
            for dealer_id, brand_id,dealer in zip(selected_ids_df["Dealerid"], selected_ids_df["Brandid"],selected_ids_df["Dealer"]):
                exec_sql = """
                SET NOCOUNT ON;
                EXEC dbo.indal_AgeingAnalysis_Automailer @BrandID = ?, @DealerID = ?;
                """
                params = (int(brand_id), int(dealer_id))
                #st.write("Executing query...", dealer)
                ...
                if len(df)>0:
                    with st.expander(f"📁  {dealer} (ID: {dealer_id})",):
                        st.subheader("Generated Ageing Data Sample")
    ...
    with tab3:
        ...
        st.session_state.selected_ids_df = selected_ids_df
        To_excel =[]
        Age_sumd =[]
        if st.button("Fetch Summary") and not selected_ids_df.empty:
            for dealer_id, brand_id,dlrname in zip(selected_ids_df["Dealerid"], selected_ids_df["Brandid"],selected_ids_df["Dealer"]):

                Age_sum = pd.read_sql_query(
                        """SELECT distinct
                        Brand,Dealer,Location,DEALERID,SUM([NM 6Mths]) AS [NM 6Mths],SUM([NM 12 Mths]) AS [NM 12 Mths],SUM([NM 24 Mths]) AS [NM 24 Mths],
                        ...
                        GROUP BY Brand,Dealer,DEALERID,Location,Dealerid,Ageing_Category) AS subquery
                        where DEALERID=?
                    GROUP BY Brand, Dealer,Location,Dealerid
                ...
                to_excel = file_for[(file_for['Ageing_Category'].notnull())&(file_for['Max_qty'].astype(float)<1.0)&(file_for['Toc_qty'].astype(float)<1.0)][[
                        ...
                        'Ageing_Category','Sugg. DiscPercentage','Dealerid']]

                To_excel.append(to_excel)
                Age_sumd.append(Age_sum)
                with st.expander(label=f"📁 Agening Summary for {dlrname}"):
                    if len(Age_sum) > 0:
                        st.header("Summary of Ageing Data for Dealer {}".format(dlrname))
                        #Age_sumd.append(Age_sum)
                        st.dataframe(Age_sum, use_container_width=True)
            
            if len(Age_sumd)>0:
                age_sum=pd.concat(Age_sumd,ignore_index=True)
                st.session_state.Age_summary=age_sum

            
            if len(To_excel)>0:
                s = pd.concat(To_excel, ignore_index=True)
                st.session_state.file_data = s
                st.download_button(
                                key=f"download_summary_{dealer_id}",
                                label=f"Download Ageing Report",
                                data=s.to_csv(index=False),
                                file_name=f"Ageing Report for.csv",
                                mime="text/csv"
                            )
    ...
    with tab4:
        selected_ids_df = st.session_state.get('selected_ids_df', pd.DataFrame())
        file_data = st.session_state.get('file_data', pd.DataFrame())
        Age_sum_df =   st.session_state.get('Age_summary',pd.DataFrame())
        ...
        if st.button("Click Here to send Mail",key="Mail send") and not selected_ids_df.empty:
            ...
            for dealer_id, brand_id, To_email, Cc_email, dealer_name in zip(merge_df["Dealerid"], merge_df["Brandid"], merge_df["To"], merge_df["CC"],merge_df["dealer"]):
                #Age_sum =  Age_sum_df[['DEALERID'].astype(str)==dealer_id]
                #st.write(Age_sum_df.columns) 
                Age_sum = Age_sum_df[Age_sum_df['DEALERID'].astype(str) == str(dealer_id)]

                to_excel = file_data[(file_data['Dealerid']==dealer_id)&
                    (file_data['Ageing_Category'].notnull())
                    ]
                [[
                        'Brand', 'Dealer', 'Location','PartNumber', 'LatestPartNumber','OrderPartNumber','Description',
                        'MRP', 'NDP', 'Stock Qty', 'Stock Value(as per NDP Price)', 'StockDate',
                ...
                #filename
                output_excel ="Suggested_Non_Moving_Report_for_" +dealer_name +" "+datetime.strftime(datetime.now(),'%Y-%b')+ ".xlsx"
                if len(to_excel) > 0:
                    t = to_excel.iloc[0,0]
                    ...
                    def fmt_money(x):
                        try:
                            x = int(float(x))  # Ensure it's an integer
                            if x < 1000:
                                return f"₹ {x}"
                            else:
                                s = str(x)
