...
#tab1, tab2, tab3, tab4 = st.tabs(["Data Availability", "Dealer Status for Report", "Summary", "Mail send"])
tab1, tab2, tab3, tab4= st.tabs(["Data Availability", "Dealer Status for Report", "Summary", "Mail send"])
...
with tab1:
    ...
    with tab2:
        ...
        selected_deale = st.multiselect("Select Dealer(s)", options=allowed, key="tab2_select_dealers")
    ...
    with tab3:
        st.subheader("Summary of Nonmoving Data")
    ...
    with tab4:    
        ...
        if st.button("Click Here to send Mail",key="Mail send") and not selected_ids_df.empty:
            ...
            #selected_ids_df = selected_ids_df.merge(merge_df, left_on='Dealerid', right_on='Dealerid', how='inner') 
            ...
            for dealer_id, brand_id, To_email, Cc_email, dealer_name in zip(merge_df["Dealerid"], merge_df["Brandid"], merge_df["To"], merge_df["CC"],merge_df["dealer"]):  
                ...
                # file_for=pd.read_sql_query(""""               
                #             select A.*,C.OrderPartNumber FROM  automation..AgeingAnalysisData_Automailer	AS a
                #             left join Part_Master c on a.Brandid=c.brandid
                #             and REPLACE(A.PartNumber,'''','')=REPLACE(c.partnumber ,'''','') WHERE A.Brandid=?
                #             AND A.Dealerid=? AND CAST(A.Addeddate AS date) = CAST(GETDATE() AS date)""",conn,
                #             params=(int(brand_id), int(dealer_id)),)
                ...
                #     lambda row: sug_dis['NM_6M_Disc'] if row['Ageing_Category'] == 'NM 6 Mths' and row['Brandid'] == sug_dis['Brandid'].values[0]
                #     else sug_dis['NM_12M_Disc'] if row['Ageing_Category'] == 'NM 12 Mths' and row['Brandid'] == sug_dis['Brandid'].values[0]
                #     else sug_dis['NM_24M_Disc'] if row['Ageing_Category'] == 'NM 24 Mths' and row['Brandid'] == sug_dis['Brandid'].values[0]
                #     else None,
                #     axis=1)
                ...
                #filename
                output_excel =Document_folder+"\ Ageing Report For " +dealer_name +" "+datetime.strftime(datetime.now(),'%Y-%m-%d')+ ".xlsx"
                if len(to_excel) > 0:
                    t = to_excel.iloc[0,0]
